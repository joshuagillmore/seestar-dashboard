...
- ~~**`Detail` button is enabled and does nothing**~~ — no target screen exists until a later slice. **Resolved 2026-08-03.** `PlanCard`'s Detail now opens that target's report on Review & QA, via the same `pendingTarget` handoff the Projects quality bar uses, and renders disabled — with the reason in its title — when the archive holds no subs for the target.

  > **Correction.** An earlier pass marked this resolved on the grounds that `ProjectCard` takes `onOpenQa`. That was wrong: this item is about `PlanCard` on **Tonight**, and its Detail button was still `<button className={styles.secondary}>Detail</button>` with no handler at all. Resolving an item by pointing at a different button on a different screen is exactly the kind of thing this file's standing checks exist to catch, and it went in unverified.
